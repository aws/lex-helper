...
def test_bedrock_invoke(model_id: str, debug: bool = False, use_converse: bool = False):
    ...
    try:
        ...
        response = invoke_bedrock(
            prompt=prompt, model_id=model_id, max_tokens=200, temperature=0.1, use_converse=use_converse
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="anthropic.claude-3-sonnet-20240229-v1:0", help="Model ID to test (default: Claude 3 Sonnet)"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--converse", action="store_true", help="Use Converse API instead of InvokeModel API")

    parser.add_argument("--example", action="store_true", help="Run the converse API with caching example")
