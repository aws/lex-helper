...
def create_button(text: str, value: Optional[str] = None) -> Button:
...
def create_buttons(texts: List[str], values: Optional[List[str]] = None) -> List[Button]:
...
def format_buttons_for_display(buttons: List[Button], style: str = "default") -> str:
    ...
    if style == "verbose":
        return "\n".join(
            f"[{button.text}] -> {button.value}" for button in buttons
        )
