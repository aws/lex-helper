...
class ReservationUtils:
    ...
    def handle_city_resolution(
        ...
        if not isinstance(iata_result, dict) or "status" not in iata_result:
            logger.warning("Invalid response from city resolution: %s", iata_result)
        ...
        if iata_result["status"] == "multiple":
            ...
            try:
                ...
                logger.error("Error setting slot values: %s", e)
                return dialog.delegate(lex_request=lex_request)
        else:
...
JSON ONLY."""
        ...
        try:
            ...
            logger.debug("Bedrock response for '%s': %s", city_input, response)
            ...
            if isinstance(response, dict) and "text" in response:
                ...
                try:
                    ...
                    logger.warning("Non-JSON response from Bedrock: %s", text_content)
            ...
            logger.error("Failed to resolve city to IATA: %s", e)
            return {"status": "error"}
