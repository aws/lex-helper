...
class ReservationUtils:
    ...
    def handle_city_resolution(
        ...
        if iata_result["status"] == "multiple":
            options_list = iata_result.get("options", [])
            logger.debug(f"Multiple airports found for {city}: {options_list}")
            
            if not options_list:
                logger.warning(f"Status is 'multiple' but options list is empty for city: {city}")
                # Fall back to asking for clarification
                message = f"I found multiple airports for {city}, but couldn't retrieve the list. Please specify the airport code (e.g., JFK, LAX)."
            else:
...
JSON ONLY."""
        ...
        try:
            ...
            if isinstance(response, dict) and 'text' in response:
                ...
                logger.debug(f"Bedrock text content: {text_content}")
                try:
